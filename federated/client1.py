...
class FlowerClient(fl.client.NumPyClient):
    ...
    def set_parameters(self, parameters):
        ...
        try:
            received_rules_raw = parameters[0].tolist()
            log.debug(f"🔹 Converted received rules to list: {received_rules_raw}")
            
            # ✅ NEW: Flatten multi-line rules
            received_rules = []
            for entry in received_rules_raw:
                for rule in entry.strip().split("\n"):
                    if rule.strip():  # ignore empty lines
                        received_rules.append(rule.strip())

            log.debug(f"📜 Flattened rule list: {received_rules}")
    ...
    def fit(self, parameters, config):
            ...
            while True:
                ...
                if best_score == None or score > best_score:
                    ...
                    if outcome == (Outcome.ALL, Outcome.NONE):
                        ...
                        learned_rules = self.stats.solution.code
                        self.current_rules = learned_rules
                        ...
                        return [rules_array], len(rule_strings), {"accuracy": 1.0}
                        

                    self.stats.register_best_program(program, conf_matrix)
                    if self.stats.solution:
                        learned_rules = self.stats.solution.code
                    else:
                        # fallback to best program so far (even if not perfect)
                        learned_rules = self.stats.best_program.code if self.stats.best_program else None
                    #if not isinstance(learned_rules, list):
                    #    learned_rules = [learned_rules]
                    #rules_array = np.array(learned_rules, dtype="<U1000")
                    #self.set_parameters([rules_array])
                    
                    #return [rules_array], len(self.current_rules), {"score": float(score)}
                # BUILD RULES
        ...
        learned_rules = self.stats.best_program.code if self.stats.best_program else None 
        log.debug(f"learned rules {learned_rules}")
        self.current_rules = learned_rules
        ...
        return [rules_array], len(self.current_rules), {}
