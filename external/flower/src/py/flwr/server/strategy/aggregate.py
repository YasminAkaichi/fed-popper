...
def aggregate_popper(
    ...
    Mirrors exactly ONE iteration of srvpopper.run_server() inner while-loop.

    Order per round:
      1. Apply constraints from previous outcome (if any)
      2. Generate next hypothesis
      3. Return it — caller (aggregate_fit) checks early stop
    """

    log.info("===== aggregate_popper =====")

    has_feedback = (
        outcome is not None
        and outcome != (None, None)
        and current_hypothesis is not None
    )

    # ---------------------------------------------------------
    # 1) Apply constraints from previous round's outcome
    #    Mirrors: build_rules → ground_rules → add_ground_clauses
    # ---------------------------------------------------------
    if has_feedback:
        ep, en = outcome  # unpack properly — never ep=en=outcome

        log.info(f"Applying constraints for outcome=({ep},{en})")
    ...
    #             if not model: break (inner while)
    #             outer for-loop then increments size
    # ---------------------------------------------------------
    with stats.duration('generate'):
        model = solver.get_model()

    if not model:
        # No model at current clause size.
        # Increment size exactly once — do NOT retry here.
        # Next Flower round will call get_model() on the updated solver.
        clause_size += 1
        solver.update_number_of_literals(clause_size)
        stats.update_num_literals(clause_size)

        log.info(f"No model — clause size bumped to {clause_size}. Will generate next round.")

        # Re-send current hypothesis so clients are not idle this round.
        # They will re-test it; feedback is discarded server-side via has_feedback guard.
        rules_arr = (
            np.array(
        ...
        return (
            [rules_arr],
            current_min_clause,   # min_clause unchanged
            current_before,       # before unchanged
            clause_size,          # bumped
            solver,
            False,
            current_hypothesis,   # hypothesis unchanged
        )

    # ---------------------------------------------------------
    # 3) Decode new hypothesis
    #    Mirrors: program, before, min_clause = generate_program(model)
    # ---------------------------------------------------------
    current_rules, before, min_clause = generate_program(model)

    log.info(f"New hypothesis ({len(current_rules)} clause(s)):")
    ...
    return (
        ...
        clause_size,
        solver,
        False,        # solved always False — aggregate_fit checks outcome
        current_rules,
    )

def aggregate_popper9april(
...
def aggregate_popperXXX(
    global_conf,           # (tp, fn, tn, fp)
    settings,
    solver,
    grounder,
    constrainer,
    ...
    """
    One federated Popper step.
    Exactly equivalent to ONE iteration of the centralized Popper while-loop.
    """

    log.info("===== FedPopper aggregate_popper =====")

    # ---------------------------------------------------------
    # 1) Decide outcome EXACTLY like Popper
    # ---------------------------------------------------------
    tp, fn, tn, fp = global_conf
    has_tested = (tp + fn + tn + fp) > 0

    if not has_tested:
        outcome = None
    else:
        outcome = decide_outcome(global_conf)
    score = calc_score(global_conf)

    log.info(
        f"Global conf: TP={tp}, FN={fn}, TN={tn}, FP={fp} | "
        f"Outcome={outcome}, Score={score}"
    )

    # ---------------------------------------------------------
    # 2) If we already had a hypothesis → add constraints
    # ---------------------------------------------------------
    if outcome is not None and current_hypothesis:
        log.debug("Building constraints from previous hypothesis")
        ...
        constraints = build_rules(
            ...
            constrainer=constrainer,
            tester=tester,
            program=current_hypothesis,   # Clause objects
            before=current_before,
            min_clause=current_min_clause,
            outcome=outcome,
        )
    ...
    # ---------------------------------------------------------
    # 3) Generate ONE hypothesis (exactly solver.get_model())
    # ---------------------------------------------------------
    model = solver.get_model()

    if not model:
        # Same behavior as Popper when hypothesis space is exhausted
        clause_size += 1
        solver.update_number_of_literals(clause_size)
        stats.update_num_literals(clause_size)
        ...
        empty_params = [np.array([], dtype="<U1000")]
        return (
            empty_params,            # new_rules (empty)
            current_min_clause,
            current_before,
            clause_size,
            solver,
    ...
    # ---------------------------------------------------------
    # 4) Decode model into a Popper program
    # ---------------------------------------------------------
    current_rules, before, min_clause = generate_program(model)
    if has_tested:
        stats.register_program(current_rules, global_conf)
    #stats.register_program(current_rules, global_conf)

    # ---------------------------------------------------------
    # 5) Solution check (Popper stopping condition)
    # ---------------------------------------------------------
    if outcome == (Outcome.ALL, Outcome.NONE):
        ...
        return (
            [rules_arr],             # parameters for Flower
            min_clause,
            before,
            clause_size,
            solver,
    ...
    # ---------------------------------------------------------
    # 6) Normal continuation (send hypothesis to clients)
    # ---------------------------------------------------------
    rules_bytes = [Clause.to_code(r) for r in current_rules]
    rules_arr = np.array(rules_bytes, dtype="<U1000")

    return (
        [rules_arr],                # parameters
        min_clause,
        before,
        clause_size,
        solver,
